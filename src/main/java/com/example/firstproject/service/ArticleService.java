...
public class ArticleService {
    ...
    public List<Article> createArticle(List<ArticleForm> dtos) {
        ...
        List<Article> articleList = dtos.stream() // dtos를 스트림화 하여 최종 결과를 articleList에 저장
                .map(dto -> dto.toEntity()) // map()으로 dto가 하나하나 올 때마다 dto.toEntity()를 수행해 매핑
                .collect(Collectors.toList()); // 매핑한 것을 리스트로 묶기
        // 2. 엔티티 묶음을 DB에 저장하기
        articleList.stream()
                .forEach(article -> articleRepository.save(article)); // article이 하나씩 올 때마다 Repository 통해 DB에 저장
        // 3. 강제 예외 발생시키기
